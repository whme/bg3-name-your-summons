...
\*Typing uses the on-screen keyboard that only Steam Input provides, so it needs [Steam Big Picture mode](https://help.steampowered.com/en/faqs/view/6BD3-BF97-2B7C-4C7B) (or Steam Input enabled for BG3) - the same limit applies to the game's own text fields.<br>
Without Big Picture, use a physical keyboard to type.
